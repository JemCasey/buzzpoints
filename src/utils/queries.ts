...
export const getCategoriesForQuestionSetQuery = db.prepare(`
    SELECT  DISTINCT category_main_slug AS category_slug
    FROM    question
    JOIN    packet_question ON question.id = question_id
    JOIN    packet ON packet_question.packet_id = packet.id
...
export const getPlayersByQuestionSetQuery = db.prepare(`
    SELECT  player.id,
            player.name,
            player.slug,
            team.name AS team_name,
...
export const getTeamByPlayerAndTournamentQuery = db.prepare(`
    SELECT  player.id,
            player.name,
            player.slug,
            team.name AS team_name,
...
export const getPlayersByTeamAndTournamentQuery = db.prepare(`
    SELECT  player.id,
            player.name,
            player.slug,
            team.name AS team_name,
...
export const getTeamsByQuestionSetQuery = db.prepare(`
    SELECT  team.id,
            team.name,
            team.slug,
            team.tournament_id,
...
export const getTossupsByTournamentQuery = db.prepare(`
    SELECT  tossup.id,
            ...
            ROUND(CAST(sum(iif(buzz.value = 20, 1, 0)) AS FLOAT) / COUNT(DISTINCT IIF(question_number <= tossups_read, game.id, null)), 3) AS superpower_rate,
            ROUND(CAST(sum(iif(buzz.value = 15, 1, 0)) AS FLOAT) / COUNT(DISTINCT IIF(question_number <= tossups_read, game.id, null)), 3) AS power_rate,
...
export const getTossupCategoryStatsQuery = db.prepare(`
    SELECT  category_main AS category,
            ...
            ROUND(CAST(sum(iif(buzz.value = 20, 1, 0)) AS FLOAT) / COUNT(DISTINCT IIF(question_number <= tossups_read, game.id || '-' || tossup.id, null)), 3) AS superpower_rate,
            ROUND(CAST(sum(iif(buzz.value = 15, 1, 0)) AS FLOAT) / COUNT(DISTINCT IIF(question_number <= tossups_read, game.id || '-' || tossup.id, null)), 3) AS power_rate,
...
export const getTossupCategoryStatsByQuestionSetQuery = db.prepare(`
    SELECT  category_main AS category,
            question.category_main_slug AS category_slug,
			question_set.slug AS set_slug,
            COUNT(DISTINCT IIF(question_number <= tossups_read, game.id, null)) AS heard,
...
export const getPlayerCategoryStatsQuery = db.prepare(`
    ...
    SELECT	buzz.player_id,
            player.name,
            category_main as category,
            sum(iif(buzz.value = 20, 1, 0)) as superpowers,
...
export const getTeamCategoryStatsByQuestionSetQuery = db.prepare(`
    SELECT  tournament.slug AS tournament_slug,
            question_set.slug AS set_slug,
            team.name,
            category_main AS category,
...
export const getBonusCategoryStatsForQuestionSetQuery = db.prepare(`
    SELECT  question.category_main AS category,
            question.category_main_slug AS category_slug,
            question_set.slug as set_slug,
            tournament.slug as tournament_slug,
...
export const getPlayerCategoryForQuestionSetLeaderboard = db.prepare(`
    WITH raw_buzzes AS (
        SELECT 	DISTINCT tossup_id,
                buzz_position,
                value
...
export const getTeamCategoryForQuestionSetLeaderboard = db.prepare(`
    SELECT  question_set.slug,
            question.category_main AS category,
            team.name,
            team.slug as teamSlug,
            COUNT(DISTINCT easy_part_direct.id) AS heard,
    ...
    WHERE   question_set.id = ?
    AND     question.category_main_slug = ?
    GROUP BY question_set.slug, category_main, team.name, team.slug
`);
...
export const getQuestionSetsQuery = db.prepare(`
    SELECT  question_set.id,
            ...
            (
                SELECT CAST(sum(iif(buzz.value = 20, 1, 0)) AS FLOAT) / COUNT(distinct tossup.id || '-' || game.id) as superpower_rate
                FROM   tossup
                JOIN   question ON tossup.question_id = question.id
                JOIN   packet_question ON question.id = packet_question.question_id
                JOIN   packet ON packet_question.packet_id = packet.id
            ...
            (
                SELECT CAST(sum(iif(buzz.value = 15, 1, 0)) AS FLOAT) / COUNT(distinct tossup.id || '-' || game.id) as power_rate
                FROM   tossup
                JOIN   question ON tossup.question_id = question.id
                JOIN   packet_question ON question.id = packet_question.question_id
                JOIN   packet ON packet_question.packet_id = packet.id
...
export const getPlayerLeaderboardForQuestionSet = db.prepare(`
    WITH raw_buzzes AS (
        SELECT 	DISTINCT tossup_id,
                buzz_position,
                value
...
export const getPlayerBuzzesForQuestionSet = db.prepare(`
    ...
    SELECT	question_set.slug as set_slug,
            ...
            tossup.answer,
			tossup.answer_primary as answer_primary,
            question.category_main AS category,
			question.category_main_slug as category_slug,
...
export const getTeamLeaderboardForQuestionSet = db.prepare(`
    WITH raw_buzzes AS (
        SELECT 	DISTINCT tossup_id,
                buzz_position,
                value
...
export const getTossupSummaryBySite = db.prepare(`
    SELECT	tournament.id as tournament_id,
            ...
            CAST(SUM(IIF(buzz.value > 0, 1, 0)) AS FLOAT) /
            (
                SELECT  count(distinct game.id)
                FROM    game
                JOIN    round ON round_id = round.id
            ...
            CAST(sum(iif(buzz.value = 20, 1, 0)) AS FLOAT) /
            (
                SELECT  count(distinct game.id)
                FROM    game
                JOIN    round ON round_id = round.id
            ...
            CAST(sum(iif(buzz.value = 15, 1, 0)) AS FLOAT) /
            (
                SELECT  count(distinct game.id)
                FROM    game
                JOIN    round ON round_id = round.id
